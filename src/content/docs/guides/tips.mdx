...
<Aside>
We believe this error occurs because the deployed code `index.ts` and the registration code `register.ts` are executed in different TypeScript compiler environments.
</Aside>
